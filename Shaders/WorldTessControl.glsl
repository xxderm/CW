...
vec2 screen_space(vec4 vertex){
    return(clamp(vertex.xy,-1.3,1.3)+1)*(screen_size*.5);
}
...
void main(){
    ...
    if(all(bvec4(
            ...
            gl_TessLevelInner[0]=mix(e1,e2,.5);
            gl_TessLevelInner[1]=mix(e0,e3,.5);
